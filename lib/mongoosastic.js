...
function Mongoosastic(schema, pluginOpts) {
  ...
  schema.statics.bulkIndex = function schemaBulkIndex(docs, cb) {
    cb = cb || nop;
    ...
    docs.forEach(function (doc) {
      serialModel = serialize(doc, mapping);
      serialModel._id = doc._id;
      _bulkBuffer.push({
        index: {
          _index: indexName,
          _type: typeName,
          _id: serialModel._id.toString()
        }
      });
      _bulkBuffer.push(serialModel);
    });
    console.log _bulkBuffer
    esClient.bulk({
      body: _bulkBuffer
    }, cb);
  };
